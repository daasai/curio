...
1. 仅允许 `main` 部署；指定部署审批人，并关闭审批人自我批准。
...
部署 workflow 先运行 `scripts/backup-db.ts`，再把当前代码（排除 `.env`、`data/`、依赖、既有备份）复制到 `.deploy-backups/<commit>`。新代码同步后由 `deploy.sh` 运行构建、内容门禁、PM2 重载和本机健康检查。
